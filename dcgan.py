...
# 损失函数: TODO
def loss(d,g):
    return -torch.log(d+1e-8)-torch.log((1-g)+1e-8)
...
def train():
    ...
    for epoch in range(flags["n_epoch"]):
        ...
        for i, (real_images, _) in enumerate(train_loader):
            ...
            G_z=G.forward(z)
            D_x=D.forward(real_images)
            D_G_z=D.forward(G_z.detach())
            # 判别器的损失: TODO
            d_loss=loss(D_x,D_G_z).mean()
            # 判别器梯度更新: TODO
            D.zero_grad()
            d_loss.backward()
            d_optimizer.step()
            ...
            D_G_z=D.forward(G_z)
            # 生成器的损失: TODO
            g_loss=-torch.log(D_G_z + 1e-8).mean()
            # 生成器梯度更新: TODO
            G.zero_grad()  
            g_loss.backward()  
            g_optimizer.step()
